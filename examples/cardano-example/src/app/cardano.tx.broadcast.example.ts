...
export async function cardanoTransactionBroadcastExample() {
  ...
  const txData = await cardanoSDK.transaction.prepareSignedTransaction(
    {
      fromUTXO: [
        {
          txHash: txHash,
  ...
  // https://apidoc.tatum.io/tag/cardanocoin#operation/cardanoBroadcast
  const { txId } = await cardanoSDK.blockchain.broadcast({
    txData: txData,
  })
