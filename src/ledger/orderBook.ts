 ...
 */
export const getActiveBuyTrades = async (id: string, pageSize: number = 50, offset: number = 0): Promise<OrderBookResponse[]> => get(`/v3/trade/buy?id=${id}&pageSize=${pageSize}&offset=${offset}`);

/**
 * For more details, see <a href="https://tatum.io/apidoc#operation/getSellTrades" target="_blank">Tatum API documentation</a>
 */
export const getActiveSellTrades = async (id: string, pageSize: number = 50, offset: number = 0): Promise<OrderBookResponse[]> => get(`/v3/trade/sell?id=${id}&pageSize=${pageSize}&offset=${offset}`);
