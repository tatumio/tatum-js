import { get, post, httpDelete } from 'packages/tatum-core/src'
 ...
 */
export const getActiveBuyTrades = async (id: string, pageSize = 50, offset = 0): Promise<OrderBookResponse[]> => get(`/v3/trade/buy?id=${id}&pageSize=${pageSize}&offset=${offset}`)

/**
 * For more details, see <a href="https://tatum.io/apidoc#operation/getSellTrades" target="_blank">Tatum API documentation</a>
 */
export const getActiveSellTrades = async (id: string, pageSize = 50, offset = 0): Promise<OrderBookResponse[]> => get(`/v3/trade/sell?id=${id}&pageSize=${pageSize}&offset=${offset}`)
