...
describe('TatumCronosSDK', () => {
  const sdk = TatumCronosSDK({ apiKey: REPLACE_ME_WITH_TATUM_API_KEY })

  describe('Wallet', () => {
    describe('Generate wallet', () => {
      walletTestFactory.generateBlockchainWallet(sdk.wallet, TEST_DATA.CRO)
    })

    describe('Address from XPUB', () => {
      walletTestFactory.generateAddressFromXpub(sdk.wallet, TEST_DATA.CRO)
    })

    describe('Private key from mnemonic', () => {
      walletTestFactory.generatePrivateKeyFromMnemonic(sdk.wallet, TEST_DATA.CRO)
    })

    describe('Address from private key', () => {
      walletTestFactory.generateAddressFromPrivateKey(sdk.wallet, TEST_DATA.CRO)
    })

    describe('Web3', () => {
      describe('Get client', () => {
        const provider = TEST_DATA.CRO?.PROVIDER

        it.skip('should return valid web3 client', async () => {
        ...
        it('should return valid web3 client with privateKey', async () => {
          const web3 = evmBasedWeb3({ blockchain: Blockchain.CRO })
          const client = web3.getClient(provider, TEST_DATA.CRO.TESTNET.ERC_721!.PRIVATE_KEY)
