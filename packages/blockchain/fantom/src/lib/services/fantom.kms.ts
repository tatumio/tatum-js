  ...
  return {
    ...
    async sign(tx: PendingTransaction, fromPrivateKey: string, provider?: string): Promise<string> {
      ...
      if (!transactionConfig.nonce) {
        transactionConfig.nonce = await ApiServices.blockchain.fantom.fantomGetTransactionCount(
