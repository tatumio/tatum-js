...
export async function bchApiExample() {
  ...
  const txHash = await bchSDK.api.bchBroadcast({
    txData: '62BD544D1B9031EFC330A3E855CC3A0D51CA5131455C1AB3BCAC6D243F65460D',
    signatureId: '1f7f7c0c-3906-4aa1-9dfe-4b67c43918f6',
  })
  const address = await bchSDK.api.bchGenerateAddress(
    'xpub6EsCk1uU6cJzqvP9CdsTiJwT2rF748YkPnhv5Qo8q44DG7nn2vbyt48YRsNSUYS44jFCW9gwvD9kLQu9AuqXpTpM1c5hgg9PsuBLdeNncid',
    0,
  )
  const privateKey = await bchSDK.api.bchGenerateAddressPrivateKey({
    mnemonic: 'urge pulp usage sister evidence arrest palm math please chief egg abuse',
    index: 0,
  })
  const wallet = await bchSDK.api.bchGenerateWallet(
  ...
  const tx = await bchSDK.api.bchGetRawTransaction(
