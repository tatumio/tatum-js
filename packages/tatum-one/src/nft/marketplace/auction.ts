...
import { get20ContractDecimals, prepareDeployAuctionSignedTransaction } from '../../transaction'
...
export const prepareAuctionUpdateFee = async (body: ChainUpdateAuctionFee, provider?: string) => {
  const params = await prepareAuctionUpdateFeeAbstraction({ ...body, chain: Currency.ONE })
...
export const prepareAuctionUpdateFeeRecipient = async (body: ChainUpdateMarketplaceFeeRecipient, provider?: string) => {
  const params = await prepareAuctionUpdateFeeRecipientAbstraction({ ...body, chain: Currency.ONE })
...
export const prepareAuctionApproveNftTransfer = async (body: ChainApproveNftTransfer, provider?: string) => {
  const params = await prepareAuctionApproveNftTransferAbstraction({ ...body, chain: Currency.ONE })
...
export const prepareAuctionCreate = async (body: ChainCreateAuction, provider?: string) => {
  const { body: validatedBody, params } = await prepareAuctionCreateAbstraction({ ...body, chain: Currency.ONE })
...
export const prepareAuctionBid = async (testnet: boolean, body: ChainInvokeAuctionOperation, provider?: string) => {
  const {
    ...
    params,
    methodName,
  } = await prepareAuctionBidAbstraction(helperGetWeb3Client, get20ContractDecimals, testnet, { ...body, chain: Currency.ONE }, provider)
...
export const prepareAuctionCancel = async (body: ChainInvokeAuctionOperation, provider?: string) => {
  const params = await prepareAuctionCancelAbstraction({ ...body, chain: Currency.ONE })
...
export const prepareAuctionSettle = async (body: ChainInvokeAuctionOperation, provider?: string) => {
  const params = await prepareAuctionSettleAbstraction({ ...body, chain: Currency.ONE })
