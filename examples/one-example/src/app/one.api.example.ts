...
export async function oneApiExample() {
  const smartContract = await oneSDK.api.oneBlockchainSmartContractInvocation({
  ...
  const address = await oneSDK.api.oneGenerateAddress('xpub6EsCk1uU6cJzqvP9CdsTiJwT2rF748YkPnhv5Qo8q44DG7nn2vbyt48YRsNSUYS44jFCW9gwvD9kLQu9AuqXpTpM1c5hgg9PsuBLdeNncid', 0)
  const privateKey = await oneSDK.api.oneGenerateAddressPrivateKey({
    index: 0,
    mnemonic: "urge pulp usage sister evidence arrest palm math please chief egg abuse"
  })
  const wallet = await oneSDK.api.oneGenerateWallet('urge pulp usage sister evidence arrest palm math please chief egg abuse')
  const balance = await oneSDK.api.oneGetBalance('0x3223AEB8404C7525FcAA6C512f91e287AE9FfE7B')
  const block = await oneSDK.api.oneGetBlock('0x5d40698ee1b1ec589035f2a39c6162287e9056868cc79d66cfb248ba9f66c3fc')
  const currentBlock = await oneSDK.api.oneGetCurrentBlock()
  const transaction = await oneSDK.api.oneGetTransaction('0xe6e7340394958674cdf8606936d292f565e4ecc476aaa8b258ec8a141f7c75d7')
  const transactionCount = await oneSDK.api.oneGetTransactionCount('0xdac17f958d2ee523a2206206994597c13d831ec7')
