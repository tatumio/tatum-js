...
import { CONTRACT_ADDRESSES, validateBody, Currency, TrcType, TransferTrxOffchain } from '@tatumio/tatum-core'
...
export const sendTronOffchainTransaction = async (testnet: boolean, body: TransferTrxOffchain) => {
  ...
  if (mnemonic && index !== undefined) {
    fromPriv =
      mnemonic && index ? await generatePrivateKeyFromMnemonic(Currency.TRON, testnet, mnemonic, index) : (fromPrivateKey as string)
