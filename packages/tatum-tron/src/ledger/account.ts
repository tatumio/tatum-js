import { post, get, httpDelete, put } from 'packages/tatum-core/src'
 ...
 */
export const getBlockedAmountsByAccountId = async (id: string, pageSize = 50, offset = 0): Promise<Blockage[]> => get(`/v3/ledger/account/block/${id}?pageSize=${pageSize}&offset=${offset}`)
 ...
 */
export const getAccountsByCustomerId = async (id: string, pageSize = 50, offset = 0): Promise<Account[]> => get(`/v3/ledger/account/customer/${id}?pageSize=${pageSize}&offset=${offset}`)

/**
 * For more details, see <a href="https://tatum.io/apidoc#operation/getAllAccounts" target="_blank">Tatum API documentation</a>
 */
export const getAllAccounts = async (pageSize = 50, offset = 0): Promise<Account[]> => get(`/v3/ledger/account?pageSize=${pageSize}&offset=${offset}`)
