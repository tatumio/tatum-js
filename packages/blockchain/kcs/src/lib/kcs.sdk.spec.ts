...
describe('TatumKcsSDK', () => {
  ...
  describe('Web3', () => {
    describe('Get client', () => {
      const provider = TEST_DATA.KCS?.PROVIDER
      ...
      it('should return valid web3 client with privateKey', async () => {
        const web3 = evmBasedWeb3({ blockchain: Blockchain.KCS })
        const client = web3.getClient(provider, TEST_DATA.KCS.TESTNET.ERC_721!.PRIVATE_KEY)
