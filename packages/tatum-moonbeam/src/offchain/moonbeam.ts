...
/**
 * Send Moonbeam transaction from Tatum Ledger account to the blockchain. This method broadcasts signed transaction to the blockchain.
...
export const sendOffchainTransaction = async (testnet: boolean, body: TransferOffchain, provider?: string) => {
  ...
  if (account.currency === Currency.GLMR) {
  ...
  try {
    return { ...(await offchainBroadcast({ txData, withdrawalId: id, currency: Currency.GLMR })), id }
  } catch (e) {
    console.error(e)
    try {
