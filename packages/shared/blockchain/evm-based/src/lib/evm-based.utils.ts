...
export const evmBasedUtils = {
  ...
  validateErc20Balance: async (client: Web3, privateKey: string, contractAddress: string, tx: TransactionConfig) => {
    const { value } = tx
    ...
    const balance = await contract.methods.balanceOf(account.address)

    if (!balance || new BigNumber(balance).isLessThan(value.toString())) {
      throw new EvmBasedSdkError({
        code: SdkErrorCode.INSUFFICIENT_FUNDS,
        error: new Error(
          `Insufficient funds burn erc20 transaction from account ${account.address} -> available balance is ${balance}, required balance is ${value.toString()}`,
        ),
      })
    }
  },
