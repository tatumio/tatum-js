...
export async function dogeLedgerAccountsExample() {
  ...
  const transactionByAccount = await dogeSDK.ledger.transaction.getAllByAccount({ id: '5e6645712b55823de7ea82f1' });
  const transactionByCUstomer = await dogeSDK.ledger.transaction.getAllByCustomer({
    id: '5e6be8e9e6aa436299950c41'
  });
  const transactionByReference = await dogeSDK.ledger.transaction.getAllByReference('5e6be8e9e6aa436299950c41');
