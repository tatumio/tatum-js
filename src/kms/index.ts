...
export const getPendingTransactionsKMSByChain = async (chain: Currency, pageSize: number = 50, offset: number = 0): Promise<TransactionKMS[]> => {
    return (await axios.get(`https://api.tatum.io/v3/ledger/kms/pending/${chain}?pageSize=${pageSize}&offset=${offset}`, {headers: {'x-api-key': process.env.TATUM_API_KEY}})).data;
};