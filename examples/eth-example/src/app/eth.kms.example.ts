...
export async function ethKmsExample() {
  ...
  const tx = await ethSDK.kms.get(pendingSignatureIds.id)
  const signedRawTx = await ethSDK.kms.sign(
    pendingSignatureIds,
    '0x254ac28e10916b3c2def004a37fec216649288ae71c8cac41faf106193263792',
  )
  await ethSDK.kms.complete(pendingSignatureIds.id, pendingSignatureIds.txId!)
  await ethSDK.kms.delete(pendingSignatureIds.id)
}
