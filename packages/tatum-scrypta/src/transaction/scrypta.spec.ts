import { TransferBtcBasedBlockchain } from '@tatumio/tatum-core'
import { prepareBlockchainSignedTransaction } from './scrypta'
/**
 * WIP: This part is still under development
 */
describe('LYRA transactions', () => {
  it('should test LYRA transaction data', async () => {
    const body = new TransferBtcBasedBlockchain()
    body.fromUTXO = [
      {
        txHash: '53faa103e8217e1520f5149a4e8c84aeb58e55bdab11164a95e69a8ca50f8fcc',
        index: 0,
        privateKey: 'cVX7YtgL5muLTPncHFhP95oitV1mqUUA5VeSn8HeCRJbPqipzobf',
      },
    ]
    body.to = [
      {
        address: 'mjJotvHmzEuyXZJGJXXknS6N3PWQnw6jf5',
        value: 0.02969944,
      },
    ]
    const txData = await prepareBlockchainSignedTransaction(true, body)
    expect(txData).toBe(
      ''
    )
  })

  it('should not test LYRA transaction data, fromAddress and fromUTXO present at the same time', async () => {
    const body = new TransferBtcBasedBlockchain()
    body.fromUTXO = [
      {
        txHash: '53faa103e8217e1520f5149a4e8c84aeb58e55bdab11164a95e69a8ca50f8fcc',
        index: 0,
        privateKey: 'cVX7YtgL5muLTPncHFhP95oitV1mqUUA5VeSn8HeCRJbPqipzobf',
      },
    ]
    body.fromAddress = [
      {
        address: 'mjJotvHmzEuyXZJGJXXknS6N3PWQnw6jf5',
        privateKey: 'cVX7YtgL5muLTPncHFhP95oitV1mqUUA5VeSn8HeCRJbPqipzobf',
      },
    ]
    body.to = [
      {
        address: 'mjJotvHmzEuyXZJGJXXknS6N3PWQnw6jf5',
        value: 0.02969944,
      },
    ]
    try {
      await prepareBlockchainSignedTransaction(true, body)
      fail('Validation did not pass.')
    } catch (e) {
      console.error(e)
    }
  })
})
