...
export async function bscNftExample() {
  ...
  const nftDeploy = (await bscSDK.nft.send.deploySignedTransaction({
    name: 'My ERC721',
    symbol: 'ERC_SYMBOL',
  ...
  const transactionData = await bscSDK.blockchain.smartContractGetAddress('BSC', nftDeploy.txId)
  ...
  const nftMinted = (await bscSDK.nft.send.mintSignedTransaction({
    tokenId,
    contractAddress,
    fromPrivateKey: senderPrivateKey,
    to: senderAddress,
  ...
  const response = await bscSDK.nft.getNFTMetadataURI(Currency.BSC, contractAddress, tokenId)
  ...
  const nftTransferred = (await bscSDK.nft.send.transferSignedTransaction({
    to: receiverAddress,
    tokenId,
    contractAddress,
    fromPrivateKey: senderPrivateKey,
  ...
  const nftBurned = (await bscSDK.nft.send.burnSignedTransaction({
    tokenId,
    contractAddress,
