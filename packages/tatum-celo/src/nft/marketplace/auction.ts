...
import { getCeloErc20ContractDecimals, prepareCeloDeployAuctionSignedTransaction } from '../../transaction'
...
export const prepareAuctionBid = async (testnet: boolean, body: InvokeAuctionOperation, provider?: string) => {
  const {
    ...
    params,
    methodName,
  } = await prepareAuctionBidAbstraction(helperGetWeb3Client, getCeloErc20ContractDecimals, testnet, body, provider)
