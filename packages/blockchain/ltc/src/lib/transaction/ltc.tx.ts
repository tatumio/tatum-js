...
export const ltcTransactions = (
  apiCalls: {
    ltcGetTxByAddress: typeof ApiServices.blockchain.ltc.ltcGetTxByAddress
    ltcBroadcast: typeof ApiServices.blockchain.ltc.ltcBroadcast
    ltcGetRawTransaction: typeof ApiServices.blockchain.ltc.ltcGetRawTransaction
  } = {
    ltcGetTxByAddress: ApiServices.blockchain.ltc.ltcGetTxByAddress,
    ltcBroadcast: ApiServices.blockchain.ltc.ltcBroadcast,
    ltcGetRawTransaction: ApiServices.blockchain.ltc.ltcGetRawTransaction,
  },
): BtcBasedTx<LtcTransactionTypes> => {
  const sendTransaction = async (body: LtcTransactionTypes): Promise<TransactionHashKMS> => {
    return apiCalls.ltcBroadcast({
      txData: await prepareSignedTransaction(body),
    })
  }

  const prepareSignedTransaction = async (body: LtcTransactionTypes): Promise<string> => {
    try {
  ...
  function verifyAmounts(tx: Transaction, body: LtcTransactionTypes) {
  ...
  const privateKeysFromAddress = async (
    ...
    for (const item of body.fromAddress) {
      const txs = await apiCalls.ltcGetTxByAddress(item.address, 50) // @TODO OPENAPI remove pageSize
  ...
  const privateKeysFromUTXO = async (
    ...
    for (const item of body.fromUTXO) {
      const tx = await apiCalls.ltcGetRawTransaction(item.txHash)
