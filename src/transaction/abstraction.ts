...
import {
    sendKlaytnStoreDataTransaction,
    sendKlaytnTransaction,
    sendKlaytnTransferErc20SignedTransaction,
} from './klaytn'
...
export const storeData = async (testnet: boolean, body: CreateRecord, provider?: string) => {
  ...
  switch (body.chain) {
    ...
    case Currency.KLAY:
      return await sendKlaytnStoreDataTransaction(testnet, body, provider);
    case Currency.XDC:
...
export const sendTransaction = async (testnet: boolean, chain: Currency,
  ...
  switch (chain) {
    ...
    case Currency.KLAY: {
      const b = body as TransferErc20;
      return b.contractAddress ? sendKlaytnTransferErc20SignedTransaction(testnet, b, provider) : sendKlaytnTransaction(testnet, b, provider);
    }
    case Currency.ONE: {
      const b = body as TransferErc20;
