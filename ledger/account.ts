...
export const blockAmount = async (id: string, block: BlockAmount): Promise<{ id: string }> => {
    ...
    return (await axios.post(`https://api.tatum.io/v3/ledger/account/block/${id}`, block, {headers: {'x-api-key': process.env.TATUM_API_KEY}})).data;
};

export const deleteBlockedAmount = async (id: string): Promise<void> => {
    await axios.delete(`https://api.tatum.io/v3/ledger/account/block/${id}`, {headers: {'x-api-key': process.env.TATUM_API_KEY}});
};

export const deleteBlockedAmountForAccount = async (id: string): Promise<void> => {
    await axios.delete(`https://api.tatum.io/v3/ledger/account/block/account/${id}`, {headers: {'x-api-key': process.env.TATUM_API_KEY}});
};

export const activateAccount = async (id: string): Promise<void> => {
    await axios.put(`https://api.tatum.io/v3/ledger/account/${id}/activate`, undefined, {headers: {'x-api-key': process.env.TATUM_API_KEY}});
};

export const deactivateAccount = async (id: string): Promise<void> => {
    await axios.put(`https://api.tatum.io/v3/ledger/account/${id}/deactivate`, undefined, {headers: {'x-api-key': process.env.TATUM_API_KEY}});
};

export const freezeAccount = async (id: string): Promise<void> => {
    await axios.put(`https://api.tatum.io/v3/ledger/account/${id}/freeze`, undefined, {headers: {'x-api-key': process.env.TATUM_API_KEY}});
};
...
export const getAccountBalance = async (id: string): Promise<AccountBalance> => {
    return (await axios.get(`https://api.tatum.io/v3/ledger/account/${id}/balance`, {headers: {'x-api-key': process.env.TATUM_API_KEY}})).data;
};