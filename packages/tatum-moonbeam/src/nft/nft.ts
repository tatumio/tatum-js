...
import { MoonbeamTx } from '../model'

export const mintNFT = (body: ChainMintErc721): Promise<TransactionHash> => mintNFTRequest({ ...body, chain: Currency.GLMR })
...
export const createNFT = async (
  ...
  return await createNFTAbstraction(mintNFTWithUri, testnet, { ...body, chain: Currency.GLMR }, file, name, description, scheme, provider)
}
...
export const prepareAddNFTMinter = async (body: ChainAddMinter, provider?: string) => {
  const params = await prepareAddNFTMinterAbstraction({ ...body, chain: Currency.GLMR })
 ...
 */
export const getNFTTransaction = async (hash: string): Promise<MoonbeamTx> => getNFTTransactionDefi<MoonbeamTx>(Currency.GLMR, hash)
...
export const getNFTTransactionsByToken = async (
  ...
  to?: number,
  sort?: Sort
): Promise<NftTransaction[]> => getNFTTransactionsByTokenDefi(Currency.GLMR, tokenId, tokenAddress, pageSize, offset, from, to, sort)
...
export const getNFTTransactionsByAddress = async (
  ...
  to?: number,
  sort?: Sort
): Promise<NftTransaction[]> => getNFTTransactionsByAddressDefi(Currency.GLMR, address, tokenAddress, pageSize, offset, from, to, sort)
...
export const getNFTsByAddress = async (contractAddress: string, address: string) => {
  return getNFTsByAddressDefi(Currency.GLMR, contractAddress, address)
}
...
export const getNFTProvenanceData = async (contractAddress: string, tokenId: string) => {
  return getNFTProvenanceDataDefi(Currency.GLMR, contractAddress, tokenId)
}
...
export const getNFTContractAddress = async (txId: string) => {
  return getNFTContractAddressDefi(Currency.GLMR, txId)
}
...
export const getNFTMetadataURI = async (contractAddress: string, tokenId: string, account?: string) => {
  return getNFTMetadataURIDefi(Currency.GLMR, contractAddress, tokenId, account)
}
...
export const getNFTImage = async (contractAddress: string, tokenId: string, account?: string) => {
  return getNFTImageDefi(Currency.GLMR, contractAddress, tokenId, account)
}
...
export const getNFTRoyalty = async (contractAddress: string, tokenId: string) => {
  return getNFTRoyaltyDefi(Currency.GLMR, contractAddress, tokenId)
}
