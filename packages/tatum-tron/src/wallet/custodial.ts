...
export const prepareTransferFromCustodialWallet = async (testnet: boolean, body: TransferFromTronCustodialAddress, provider?: string) => {
    ...
    return prepareTransferFromCustodialWalletAbstract(
        ...
        body,
        getTronTrc20ContractDecimals,
        (_r, _provider) => prepareTronSmartContractInvocation(_r, feeLimit as number, from, _provider),
        SmartContractMethodInvocation,
        18,
        TransferFromCustodialAddress,
        provider
 ...
 * @returns {txId: string} Transaction ID of the operation, or signatureID in case of Tatum KMS
 */
export const prepareBatchTransferFromCustodialWallet = async (testnet: boolean, body: TransferFromTronCustodialAddressBatch, provider?: string) => {
    const {feeLimit, from} = body;
    return prepareBatchTransferFromCustodialWalletAbstract(
        testnet,
        body,
        getTronTrc20ContractDecimals,
        (_r, _provider) => prepareTronCustodialTransferBatch(_r, feeLimit as number, from, _provider),
        SmartContractMethodInvocation,
        18,
        TransferFromTronCustodialAddressBatch,
        provider
