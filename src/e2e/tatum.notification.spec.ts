...
describe('notification', () => {
  beforeAll(async () => {
    const tatum = await TatumSDK.init({network: Network.ETHEREUM, verbose: true, retryCount: 10})
    ...
    describe('IP auth', () => {
      describe('Address Event', () => {
        it.each(Object.values(AddressEventNetworks))('OK %s', async (network: Network) => {
          const tatum = await TatumSDK.init({ network, verbose: true, retryCount: 10 })
  ...
  describe('deleteSubscription', () => {
    it('OK', async () => {
      const tatum = await TatumSDK.init({ network: Network.ETHEREUM_SEPOLIA, verbose: true, retryCount: 10 })
