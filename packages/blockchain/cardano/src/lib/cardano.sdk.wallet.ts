...
export const cardanoWallet = (args: SDKArguments, apiCalls: {
  rpcCall: typeof NodeRpcService.nodeJsonPostRpcDriver,
} = {
  rpcCall: NodeRpcService.nodeJsonPostRpcDriver,
}) => {

  const deriveAddressFromPubKey = (key: string, testnet: boolean) => apiCalls.rpcCall('ADA', {
    ...
    metadata: {},
  }, args.apiKey, 'construction/derive').then(r => r.account_identifier.address)

  return {
    generateAddressFromXPub: async (xpub: string, i: number, options: { testnet: boolean }): Promise<string> => {
    ...
    generatePrivateKeyFromMnemonic: async (mnemonic: string, i: number): Promise<string> => {
      const privateKeyHex = await cardanoUtils.bip32Ed25519.fromBip39Entropy(Buffer.from(mnemonicToEntropy(mnemonic), 'hex'), '')
      const key = await cardanoUtils.bip32Ed25519.derivePrivateKey(privateKeyHex, [harden(1852), harden(1815), harden(0), harden(0), i])
    ...
    generateWallet: async (mnemonic?: string): Promise<Wallet> => {
      ...
      const privateKeyHex = await cardanoUtils.bip32Ed25519.fromBip39Entropy(Buffer.from(mnemonicToEntropy(m), 'hex'), '')
      const key = await cardanoUtils.bip32Ed25519.derivePrivateKey(privateKeyHex, [harden(1852), harden(1815), harden(0), harden(0)])
      return { mnemonic: m, xpub: await cardanoUtils.bip32Ed25519.getBip32PublicKey(key) }
    },
    generateAddressFromPrivateKey: async (privateKey: string, options: { testnet: boolean }): Promise<string> => {
