...
export const prepareCeloMintCashbackErc721SignedTransaction = async (testnet: boolean, body: CeloMintErc721, provider?: string) => {
  ...
  if (contractAddress && feeCurrency) {
    const feeCurrencyContractAddress = getFeeCurrency(feeCurrency, testnet)
    // @ts-ignore
    const contract = new (new Web3()).eth.Contract(erc721CashbackTokenABI, contractAddress.trim())
    const cb: string[] = []
 ...
 * @returns raw transaction data in hex, to be broadcasted to blockchain.
 */
export const prepareCeloMintMultipleErc721ProvenanceSignedTransaction = async (testnet: boolean, body: CeloMintMultipleErc721, provider?: string) => {
...
export const prepareCeloTransferErc721SignedTransaction = async (testnet: boolean, body: CeloTransferErc721, provider?: string) => {
  ...
  // @ts-ignore
  const contract = new (new Web3()).eth.Contract(provenance ? erc721Provenance_abi : erc721CashbackTokenABI, contractAddress.trim());
