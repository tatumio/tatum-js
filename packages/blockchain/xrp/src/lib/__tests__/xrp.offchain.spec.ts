...
describe('XrpSDK - offchain', () => {
    ...
    describe('send offchain transaction', () => {
        it ('valid', async () => {
            ...
            testHelper.expectMockCalled(mockedApi.offChain.withdrawal.broadcastBlockchainTransaction, [{ txData: VALID_TX_DATA, withdrawalId: '1', currency: Currency.XRP }])
        })

        it('!fee', async () => {
            mockGetAccountInfo()
      
            await expect(offchainService.sendOffchainTransaction(
                {
                    senderAccountId: ACCOUNT,
                    account: ACCOUNT,
                    address: ACCOUNT,
